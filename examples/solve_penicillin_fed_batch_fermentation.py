"""Solve the Penicillin Fed-Batch Fermentation problem.

Source:
    J. Fu et al., "Local optimization of dynamic programs with guaranteed
    satisfaction of path constraints," Automatica, 2015, Appendix A.1.

This version enforces the substrate-concentration path constraint.
"""
...
DISPLAY_NAME = "Penicillin Fed-Batch Fermentation"
NUM_INTERVALS = 40
INITIAL_STATE = np.array([1.0, 0.2, 0.001, 250.0])
INITIAL_CONTROL = 1.0
...
class PenicillinFedBatchFermentationModel(BaseDynamicModel):
    ...
    def setup_model(self) -> None:
        ...
        x = self.x_sym
        u = self.u_sym[0]
        denominator_l = x[1] + k_l * x[0]
        denominator_p = k_p + x[1] + x[1] ** 2 / k_i
        self.ode_expr = ca.vertcat(
            mu * x[0] * x[1] / denominator_l
            - u * x[0] / x[3],
            u * (s_l - x[1]) / x[3]
            - m_x * x[0]
            - theta * x[0] * x[1] / (y_p * denominator_p)
            - mu * x[0] * x[1] / (y_xs * denominator_l),
            theta * x[0] * x[1] / denominator_p
            - u * x[2] / x[3]
            - k_xp * x[2],
            u,
        )
...
def build_ocp() -> OptimalControlProblem:
    model = PenicillinFedBatchFermentationModel()
    ocp = OptimalControlProblem(model)
    x = model.x_sym
    ocp.set_time_horizon(tf=40.0)
    ocp.set_objective(mayer=-x[2])
    ocp.set_variable_bounds("u", lb=0.0, ub=10.0)
    ocp.set_variable_bounds(
        "x",
        lb=[1e-8, 1e-8, 0.0, 1e-8],
    )
    ocp.add_initial_constraint(x, INITIAL_STATE)
    ocp.add_path_constraint(
        x[1] - 0.5,
        name="penicillin_fed_batch_fermentation_substrate_limit",
    )
    return ocp
...
def build_initial_guess(
    ...
    step = build_interval_integrator(
        ...
        ca.SX.zeros(0, 1),
        [1.0],
        name="penicillin_fed_batch_fermentation_initial_guess",
...
def postprocess_solution(result: SMSAlgorithmResult) -> None:
    """Densely reconstruct, check, save, and plot the fermentation."""
    ...
    propagator = build_interval_integrator(
        ...
        ca.SX.zeros(0, 1),
        local_points[1:],
        name="penicillin_fed_batch_fermentation_postprocess",
    ...
    for interval_index in range(transcription.num_intervals):
        ...
        residual_segments.append(
            interval_states[1, :point_count] - 0.5
        )
    ...
    solution_path = (
        OUTPUT_DIRECTORY
        / "penicillin_fed_batch_fermentation_solution.npz"
    )
    plot_path = (
        OUTPUT_DIRECTORY
        / "penicillin_fed_batch_fermentation_path_constraint.png"
    )
    np.savez(
        solution_path,
    ...
    axis.plot(time, path_residual, label=r"$x_2(t)-0.5$")
    axis.axhline(0.0, color="black", linestyle="--", linewidth=1.0)
    axis.set(
        title="Penicillin fed-batch fermentation path constraint",
