"""Solve the State-Constrained Van der Pol Oscillator problem.

Source:
    T. W. C. Chen and V. S. Vassiliadis, "Inequality path constraints in
    optimal control: A finite iteration epsilon-convergent scheme based on
    pointwise discretization," Journal of Process Control, 2005, Section 6.1.
"""
...
DISPLAY_NAME = "State-Constrained Van der Pol Oscillator"
NUM_INTERVALS = 20
INITIAL_STATE = np.array([0.0, 1.0])
INITIAL_CONTROL = 0.35
...
class StateConstrainedVanDerPolOscillatorModel(BaseDynamicModel):
    """Controlled Van der Pol oscillator."""

    def setup_model(self) -> None:
        self.x_sym = ca.SX.sym("x", 2)
        self.u_sym = ca.SX.sym("u")
        x_0, x_1 = self.x_sym[0], self.x_sym[1]
        u = self.u_sym[0]
...
def build_ocp() -> OptimalControlProblem:
    model = StateConstrainedVanDerPolOscillatorModel()
    ocp = OptimalControlProblem(model)
    x, u = model.x_sym, model.u_sym[0]
    ocp.set_time_horizon(tf=5.0)
    ocp.set_objective(lagrange=u**2 + x[0] ** 2 + x[1] ** 2)
    ocp.set_variable_bounds("u", lb=-0.3, ub=1.0)
    ocp.add_initial_constraint(x, INITIAL_STATE)
    ocp.add_path_constraint(
        -x[0] - 0.4,
        name="state_constrained_van_der_pol_oscillator_state_limit",
    )
    return ocp
...
def build_initial_guess(
    ...
    step = build_interval_integrator(
        ...
        ca.SX.zeros(0, 1),
        [1.0],
        name="state_constrained_van_der_pol_oscillator_initial_guess",
...
def postprocess_solution(result: SMSAlgorithmResult) -> None:
    """Densely reconstruct, check, save, and plot the oscillator."""
    ...
    propagator = build_interval_integrator(
        ...
        ca.SX.zeros(0, 1),
        local_points[1:],
        name="state_constrained_van_der_pol_oscillator_postprocess",
    ...
    for interval_index in range(transcription.num_intervals):
        ...
        residual_segments.append(
            -interval_states[0, :point_count] - 0.4
        )
    ...
    solution_path = (
        OUTPUT_DIRECTORY
        / "state_constrained_van_der_pol_oscillator_solution.npz"
    )
    plot_path = (
        OUTPUT_DIRECTORY
        / "state_constrained_van_der_pol_oscillator_path_constraint.png"
    )
    np.savez(
        solution_path,
    ...
    axis.plot(time, path_residual, label=r"$-x_1(t)-0.4$")
    axis.axhline(0.0, color="black", linestyle="--", linewidth=1.0)
    axis.set(
        title="Van der Pol oscillator state constraint",
